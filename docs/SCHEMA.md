...
Each Essentia sidecar JSON includes two fields for stale-row detection:

| Field | Source | Purpose |
|---|---|---|
| `version` | `EXTRACTOR_VERSION` in `scripts/extract_essentia.py` | Bumped when extraction output changes meaningfully (e.g. 1.0 → 1.1: spectral descriptors switched from full-track FFT to frame-wise means). |
| `schema_hash` | `SCHEMA_HASH` in `src/recommender/feature_converter.py` | Short SHA-256 of `AXIS_NAMES` tuple. Changes when the feature layout changes, even if the version string is not bumped. |

The ingest pipeline (`ingest_pipeline.py`) compares both fields against the current
values. A row is re-extracted when:

- `feature_json` is NULL, OR
- `version` differs from `EXTRACTOR_VERSION`, OR
- both `schema_hash` is present in the stored sidecar and in the current code, and they differ.

Legacy sidecars (pre-`schema_hash`) with a matching `version` are treated as
current — the missing hash is not treated as a mismatch.

---

## Playlist JSON Output (`branch_playlist.json`)

Generated by `playlist_writer.py`. Structure:

```json
{
  "seed": {
    "id": 5,
    "file": "/path/to/seed.mp3",
    "title": "Seed Track",
    "artist": "Artist"
  },
  "playlist": [
    {
      ...
      "band": "Near",
      "distance": 0.1234,
      "reason": "Close timbral neighbour"
    }
  ]
}
```
...
| Field | Type | Notes |
|---|---|---|
| `seed.id` | int | Database row ID of the seed track. |
| `seed.file` | string | Absolute path to the seed audio file. |
| `playlist[].id` | int | Database row ID of the track. |
| `playlist[].file` | string | Absolute path to the audio file. Mirrors `seed.file` — added so verification commands (e.g. `jq -r '.playlist[].file'`) can resolve paths without the database. |
| `playlist[].band` | string | `"Near"`, `"Mid"`, or `"Far"` — the distance band this track was drawn from. |
| `playlist[].distance` | float | Cosine distance from the previous track (seed for position 1). |
